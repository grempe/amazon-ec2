...
context "An EC2 instance " do
  ...
  specify "method get_console_output should raise an exception when called without nil/empty string arguments" do
    lambda { @ec2.confirm_product_instance() }.should.raise(AWSAPI::ArgumentError)
    lambda { @ec2.confirm_product_instance(:product_code => "774F4FF8", :instance_id => nil) }.should.raise(AWSAPI::ArgumentError)
    lambda { @ec2.confirm_product_instance(:product_code => "774F4FF8", :instance_id => "") }.should.raise(AWSAPI::ArgumentError)
    lambda { @ec2.confirm_product_instance(:product_code => nil, :instance_id => "i-10a64379") }.should.raise(AWSAPI::ArgumentError)
    lambda { @ec2.confirm_product_instance(:product_code => "", :instance_id => "i-10a64379") }.should.raise(AWSAPI::ArgumentError)
  end
