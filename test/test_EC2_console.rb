...
context "The EC2 console " do
  ...
  specify "method get_console_output should raise an exception when called without nil/empty string arguments" do
    lambda { @ec2.get_console_output() }.should.raise(AWSAPI::ArgumentError)
    lambda { @ec2.get_console_output(:instance_id => nil) }.should.raise(AWSAPI::ArgumentError)
    lambda { @ec2.get_console_output(:instance_id => "") }.should.raise(AWSAPI::ArgumentError)
  end
