...
context "EC2 elastic IP addresses " do
  ...
  specify "method associate_address should reject bad arguments" do
    ...
    lambda { @ec2.associate_address( :instance_id => "i-2ea64347", :public_ip => "67.202.55.255" ) }.should.not.raise(AWSAPI::ArgumentError)
    lambda { @ec2.associate_address() }.should.raise(AWSAPI::ArgumentError)
    lambda { @ec2.associate_address( :instance_id => nil ) }.should.raise(AWSAPI::ArgumentError)
    lambda { @ec2.associate_address( :public_ip => "" ) }.should.raise(AWSAPI::ArgumentError)
  end
  ...
  specify "method disassociate_address should reject bad arguments" do
    ...
    lambda { @ec2.disassociate_address( :public_ip => "67.202.55.255" ) }.should.not.raise(AWSAPI::ArgumentError)
    lambda { @ec2.disassociate_address() }.should.raise(AWSAPI::ArgumentError)
    lambda { @ec2.disassociate_address( :public_ip => "" ) }.should.raise(AWSAPI::ArgumentError)
  end
