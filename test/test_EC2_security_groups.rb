...
context "EC2 security groups " do
  ...
  specify "method create_security_group should reject bad arguments" do
    ...
    lambda { @ec2.create_security_group( :group_name => "WebServers", :group_description => "Web" ) }.should.not.raise(AWSAPI::ArgumentError)
    lambda { @ec2.create_security_group() }.should.raise(AWSAPI::ArgumentError)

    # :group_name can't be nil or empty
    lambda { @ec2.create_security_group( :group_name => "", :group_description => "Web" ) }.should.raise(AWSAPI::ArgumentError)
    lambda { @ec2.create_security_group( :group_name => nil, :group_description => "Web" ) }.should.raise(AWSAPI::ArgumentError)

    # :group_description can't be nil or empty
    lambda { @ec2.create_security_group( :group_name => "WebServers", :group_description => "" ) }.should.raise(AWSAPI::ArgumentError)
    lambda { @ec2.create_security_group( :group_name => "WebServers", :group_description => nil ) }.should.raise(AWSAPI::ArgumentError)
  end
  ...
  specify "method delete_security_group should reject bad arguments" do
    ...
    lambda { @ec2.delete_security_group( :group_name => "WebServers" ) }.should.not.raise(AWSAPI::ArgumentError)
    lambda { @ec2.delete_security_group() }.should.raise(AWSAPI::ArgumentError)

    # :group_name can't be nil or empty
    lambda { @ec2.delete_security_group( :group_name => "" ) }.should.raise(AWSAPI::ArgumentError)
    lambda { @ec2.delete_security_group( :group_name => nil ) }.should.raise(AWSAPI::ArgumentError)
  end
  ...
  specify "method describe_security_groups should reject bad arguments" do
    @ec2.stubs(:make_request).with('DescribeSecurityGroups', {"GroupName.1"=>"WebServers"}).
      ...
      lambda { @ec2.describe_security_groups( :group_name => "WebServers" ) }.should.not.raise(AWSAPI::ArgumentError)
